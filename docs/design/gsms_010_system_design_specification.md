...
* **`MwlGeneratorService`**:
  ...
  - **MWL Allocation Strategy**: Selects modalities randomly from available modalities. Sequentially cycles through template series per modality in round-robin order for subsequent MWL entries. In non-synthetic mode (`GOSMART_MS_SYNTHETIC_MODE=false`), assigns exact slice counts from the chosen template series.
* **`DicomGeneratorService`**:
  ...
  - **Non-Synthetic Exact Delivery Mode**: Delivers complete series matching the exact slice count of the selected template series. Preserves original slice pixel arrays and slice metadata. When burn-in text overlay is enabled, text annotations are drawn directly on top of the original slice pixel matrix.
  - **Synthetic Cyclic Rotation Mode**: In synthetic mode (`GOSMART_MS_SYNTHETIC_MODE=true`), generates slice volumes conforming to `min_slices`/`max_slices` bounds with cyclic slice rotation `(i - 1) % M`. Supports single compressed frame cloning when combined with stress mode (`GOSMART_MS_STRESS=true`).
  ...
  - **Transfer Syntax Encoding & Compression**: Employs a raw-first synthesis pipeline that generates the uncompressed image matrix (texture + 4-segment dynamic range test pattern + burned-in text) and subsequently performs in-place compression to `JPEG2000Lossless`, `JPEG2000` (Lossy with `j2k_cr=[10]`), `RLELossless`, or `JPEGBaseline8Bit`, resolving tag VR ambiguities for explicit transfer syntax compliance (`SmallestImagePixelValue`/`LargestImagePixelValue` set to VR `US`) and preserving deterministic UIDs (`generate_instance_uid=False`). Output files are persisted under `test_output/` for PACS viewer verification.

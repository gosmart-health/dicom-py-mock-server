...
* **Verification Methods:**
  ...
  - Verify non-synthetic exact slice count delivery, sequential round-robin template picking per modality, original pixel data preservation with direct burn-in annotation, and synthetic mode cyclic slice rotation with stress-mode single-frame compression cloning (`test_template_datasets.py`).

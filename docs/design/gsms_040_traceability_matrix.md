...
| **REQ-FUN-023** | Transfer Syntax Encoding & Raw-First Pipeline | `src/dicom_py_mock_server/services/generator.py` & `scp.py` | HAZ-001 / HAZ-003 | Raw matrix synthesis, in-place compress, UID retention, context advertisement | Unit test `test_generator.py::test_transfer_syntax_swapping_raw_jpeg_jpeg2000` | Pass |
| **REQ-FUN-024** | DICOMweb QIDO-RS Query Services | `src/dicom_py_mock_server/services/dicomweb.py` & `api/dicomweb_routes.py` | HAZ-004 | Standard DICOM JSON serialization & query filtering | Integration test `test_dicomweb.py::test_qido_search_studies_all` | Pass |
| **REQ-FUN-025** | DICOMweb WADO-RS Retrieve & Transfer Syntax Negotiation | `src/dicom_py_mock_server/services/dicomweb.py` & `api/dicomweb_routes.py` | HAZ-001 / HAZ-003 | Multipart serialization & on-the-fly syntax transcoding | Integration test `test_dicomweb.py::test_wado_retrieve_transfer_syntax_negotiation` | Pass |
| **REQ-FUN-026** | DICOMweb WADO-RS Rendered Views & WADO-URI | `src/dicom_py_mock_server/services/dicomweb.py` & `api/dicomweb_routes.py` | HAZ-004 | Normalized 8-bit image rendering & legacy WADO-URI route | Integration test `test_dicomweb.py::test_wado_rendered_jpeg_and_png` | Pass |
| **REQ-FUN-027** | High-Throughput Stress Mode (`GOSMART_MS_STRESS`) | `src/dicom_py_mock_server/services/generator.py`, `dicomweb.py`, `scp.py` | HAZ-001 / HAZ-009 | Single frame compression, demographics overlay only, syntax reuse | Integration test `test_stress_mode.py::test_create_instances_from_mwl_stress_mode_reuses_compressed_frame` | Pass |
| **REQ-FUN-028** | Multi-Slice Template Subfolder Loading & Modality Purity | `src/dicom_py_mock_server/services/mwl_generator.py` & `models/template.py` | HAZ-006 | Subfolder validation, modality tag discovery, purity check, non-image exclusion | Unit test `test_template_datasets.py::test_mixed_modality_folder_raises_error` | Pass |
| **REQ-FUN-029** | Non-Synthetic Exact Delivery & Synthetic Slice Modes | `src/dicom_py_mock_server/services/generator.py` & `mwl_generator.py` | HAZ-001 / HAZ-009 | Exact slice delivery, round-robin template picking, template study description preservation, pixel preservation, cyclic rotation | Unit test `test_template_datasets.py::test_create_instances_from_mwl_non_synthetic_exact_delivery` & `test_non_synthetic_preserves_template_study_description_and_does_not_swap_with_mockups` | Pass |

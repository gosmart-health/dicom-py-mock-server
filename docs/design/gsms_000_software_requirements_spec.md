...
| **REQ-FUN-027** | High-Throughput Stress Mode (`GOSMART_MS_STRESS`) | The system SHALL support an optimized stress mode controlled via `GOSMART_MS_STRESS=true/false` where the compressed frame payload is computed once per study/series, patient demographics are burned into the base matrix while slice number overlay is omitted, and the target transfer syntax is negotiated at association start (DIMSE) or derived from the first image request (WADO-RS) and reused across all instances. | High |
| **REQ-FUN-028** | Multi-Slice Template Subfolder Loading & Modality Purity | The system SHALL load multi-slice DICOM template series exclusively from subfolders under `templates/`, reject standalone files in the template root directory with `ValueError`, dynamically detect modality from DICOM tag `(0008,0060)`, enforce folder modality purity (rejecting mixed modalities with `ValueError`), exclude non-image objects (missing `PixelData`, PR, SR, private raw objects), group slices by `SeriesInstanceUID`, and sort slices by `(InstanceNumber, SliceLocation, ImagePositionPatient[2])`. | High |
| **REQ-FUN-029** | Non-Synthetic Exact Delivery & Synthetic Slice Modes | The system SHALL support configurable synthetic mode via `GOSMART_MS_SYNTHETIC_MODE=true/false` (default `false`). In non-synthetic mode (default), the system SHALL deliver complete series with exact template slice counts, allocate template series sequentially per modality in round-robin order for MWL entries, preserve original template Study Description without swapping with mock up values, and preserve native slice pixels (rendering burned-in annotations directly on original slice pixels if enabled). In synthetic mode, the system SHALL generate slice volumes bounded by `min_slices`/`max_slices`, rotate slices cyclically `(i - 1) % M`, generate synthetic modality-appropriate study descriptions when omitted, and support single-frame compression cloning in stress mode. | High |
| **REQ-FUN-030** | HL7 v2 MLLP ORM^O01 Ingestion & MWL Mapping | The system SHALL provide a zero-dependency HL7 v2 Minimal Lower Layer Protocol (MLLP) TCP server (default port 2575) to ingest `ORM^O01` radiology order messages, map demographic and procedure elements into active DICOM Modality Worklist (MWL) entries without modification (no anonymization or synthetic suffixes), and return standard MLLP-framed `ACK^O01` responses. | High |
| **REQ-FUN-031** | FHIR R4/R5 ServiceRequest Bundle Ingestion | The system SHALL provide REST endpoints (`POST /api/v1/fhir_service_request`, `POST /api/v1/fhir/Bundle`, `POST /api/v1/fhir/ServiceRequest`) accepting FHIR R4/R5 Bundles or ServiceRequests to map demographics and procedure codes into active MWL entries without modifying raw values. | High |
| **REQ-FUN-032** | Modality Template Availability Validation | The system SHALL inspect available DICOM template modalities prior to ingesting HL7 or FHIR orders and SHALL reject order creation with an error response (HL7 `AE` / HTTP `422`) if template images for the requested modality are absent. | High |
| **REQ-FUN-033** | Order Cancellation & Active MWL Purge | The system SHALL support order cancellation via HL7 (`ORC-1` in `CA`, `OC`, `DC`) and FHIR (`status` in `revoked`, `entered-in-error`), immediately purging matching entries from the active Modality Worklist. | High |
| **REQ-FUN-034** | DICOMweb STOW-RS Storage & Retrieval Services | The system SHALL provide standard DICOMweb STOW-RS store endpoints (`POST /dicomweb/studies` and `POST /dicomweb/studies/{studyUID}`) accepting `multipart/related; type="application/dicom"` or raw datasets, compose Part-10 DICOM files into the configured received directory (`GOSMART_MS_RECEIVED_DIR`, default `./received`), retain received instances in memory for immediate query and retrieval via QIDO-RS, WADO-RS, and DIMSE C-FIND/C-MOVE, support configurable duplicate handling (`accept`, `warn`, `reject` via `GOSMART_MS_STOW_DUPLICATE_HANDLING`), and return standard DICOM JSON responses (`ReferencedSOPSequence` and `FailedSOPSequence`). | High |
| **REQ-FUN-035** | MCP Dynamic Server Configuration Management | The system SHALL expose Model Context Protocol (MCP) tools (`get_config`, `update_config` / `set_config`) to inspect and dynamically modify any server configuration parameter defined in `AppConfig` at runtime across services. | High |
| **REQ-FUN-036** | MCP Clinical Scanning Order Generation | The system SHALL expose an MCP tool (`generate_scanning_order`) to generate a clinical imaging scanning order (MWL scheduled procedure step) requiring patient name, patient ID, accession number, and modality, with an optional study description phrase. | High |
| **REQ-FUN-037** | MCP Active Worklist Inspection & Order Purge | The system SHALL expose MCP tools (`get_worklist` / `list_mwl_entries`, `remove_accession_number` / `remove_mwl_entry`) allowing AI agents and users to view active worklist entries ("show me current worklist") and remove/cancel entries by accession number ("remove accession number <value>"). | High |
...
| **REQ-REG-006** | HL7 v2.3/v2.5 & FHIR R4/R5 Standards | Ingested HL7 v2 messages SHALL conform to MLLP framing standards (0x0B start, 0x1C 0x0D end) and standard ORM^O01 / ACK^O01 segment structures; FHIR payloads SHALL conform to HL7 FHIR Release 4 / Release 5 JSON resource specifications. |
